...
from .tl_utils import contiguous
...
def chunk_mlstm_fwd_kernel_C(
    ...
    for i_t in range(NT):
        ...
        p_n = tl.make_block_ptr(n + i_bC * s_n_h + i_t * K, (K,), (1,), (i_k * BK,), (BK,), (0,))
    ...
    if STORE_FINAL_STATE:
        ...
        p_n = tl.make_block_ptr(final_n + i_bC * K, (K,), (1,), (i_k * BK,), (BK,), (0,))
...
def chunk_mlstm_fwd_kernel_h(
    ...
    p_m_total = tl.make_block_ptr(m_total + T * i_bC, (T,), (1,), (i_t * BT,), (BT,), (0,))
...
def chunk_mlstm_bwd_kernel_dC(
    ...
    for i_t in range(NT - 1, -1, -1):
        ...
        # [BK, BT]
        b_q = tl.load(p_q, boundary_check=(0, 1))
        b_q = (b_q * scale * tl.math.exp2(b_f + b_m_p - b_m_total)[None, :]).to(b_q.dtype)
        # [BT, V]
    ...
    tl.store(p_initial_dC, b_dC.to(p_initial_dC.dtype.element_ty), boundary_check=(0, 1))
...
def chunk_mlstm_bwd_kernel_dqkvif(
    ...
    for i_v in range(tl.cdiv(V, BV)):
        ...
        # [BT, BV]
        b_dv = tl.dot(b_k, b_dC, allow_tf32=False) * tl.math.exp2(b_i - b_f + b_f_last - b_m_next)[:, None] + tl.dot(
            b_s.to(b_q.dtype) / b_norm[None, :], b_dh, allow_tf32=False
        )
...
def mLSTMFunc(chunk_size, save_states: bool = True):
    class mLSTMFunction(torch.autograd.Function):
        ...
        @custom_fwd
        @contiguous
        def forward(ctx, q, k, v, i, f, initial_C, initial_n, initial_m, output_final_state):
            ...
            if output_final_state:
                final_C = q.new_empty(B, H, K, V, dtype=torch.float32, requires_grad=False)
            ...
            if save_states:
                ctx.save_for_backward(q, k, v, C, f, i, m, m_total, norm, final_m, initial_C, initial_n, f_orig)
            else:
        ...
        def backward(ctx, dh, final_dC=None, final_dn=None, final_dm=None):
            if save_states:
                q, k, v, C, f, i, m, m_total, norm, final_m, initial_C, initial_n, f_orig = ctx.saved_tensors
            else:
                (
                    q,
                    k,
            ...
            initial_dC = q.new_empty(B, H, K, V, dtype=torch.float32, requires_grad=False)
