...
def create_layer(
    ...
    **kwargs,
    ...
    # if we create multiple layers with the same config (e.g. multiple transformer blocks), we need to make sure that
    #  the config is created only once.
    ...
    # assign the config dataclass to kwargs (used for printing the full config)
    if set_cfg_kwargs:
        if nameandkwargs is not None:
