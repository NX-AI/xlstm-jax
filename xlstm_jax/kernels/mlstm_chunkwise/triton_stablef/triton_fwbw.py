...
def _mlstm_chunkwise_fwbw_generator(
    ...
    def forward(
        ...
        matC_states, _, scaM_states = all_states
        if recompute_states_in_bw:
            matC_states = None
...
def _get_chunkwise_fwbw_kernel(autocast_kernel_dtype: jnp.dtype, **kwargs) -> Callable:
    ...
    Args:
        autocast_kernel_dtype: The dtype to use for the kernel computation. All inputs arguments up to vecF and vecI
            are cast to this dtype. vecF is automatically casted to float32 in the kernels.
...
def mlstm_chunkwise_triton_stablef(
    q: jax.Array,
    k: jax.Array,
    v: jax.Array,
    i: jax.Array,
