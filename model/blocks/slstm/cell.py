...
class sLSTMCellConfig:
    ...
    def defines(self):
        return (
            ...
            + (
                [
                    "-DSLSTM_GRADIENT_RECURRENT_CLIPVAL_VALID=true",
            ...
            + (
                [
                    "-DSLSTM_FORWARD_CLIPVAL_VALID=true",
...
class sLSTMCellBase(nn.Module):
    ...
    def _get_state(
        self, input: torch.Tensor, state: torch.Tensor | None = None
    ) -> torch.Tensor:
        if state is None:
...
def sLSTMCellFuncGenerator(training, config: sLSTMCellConfig):
    ...
    class sLSTMCellFunction(torch.autograd.Function):
        ...
        @conditional_decorator(
            _cfg["enable_automatic_mixed_precision"], torch.cuda.amp.custom_fwd
        )
        ...
        @once_differentiable
        @conditional_decorator(
            _cfg["enable_automatic_mixed_precision"], torch.cuda.amp.custom_bwd
        )
