...
def causal_forget_matrix(forget_gates):
    ...
    mask = torch.tril(
        torch.ones([1, S, S], dtype=torch.bool, device=forget_gates.device), diagonal=-1
    )
    forget_matrix = fgates[:, :, None].repeat((1, 1, S))
    forget_matrix = forget_matrix.masked_fill(~mask, 0).cumsum(dim=1)
    mask = torch.tril(
        torch.ones(S, S, device=forget_gates.device, dtype=bool), diagonal=0
    )
...
class mLSTMfwbwConfig:
    ...
    stabilize_correctly: bool = (
        False  # this is only needed, if the GroupNorm is omitted
    )
    scale = None
...
def mLSTMTorchFunction(config: mLSTMfwbwConfig):
    ...
    class mLSTMTorchFunc(torch.autograd.Function):
        ...
        def forward(ctx, q, k, v, i, f, initial_C, initial_n, initial_m):
            ...
            C = T // S
            use_chunkwise = (
                _cfg["chunk_size"] is not None
                or _cfg["return_state"]
                or _cfg["use_initial_state"]
            )
            ...
            if use_chunkwise:
                G_stabilizer = torch.maximum(
                    G_stabilizer, m[:, :, :-1, None] + backward_gates
                )
            ...
            M_norm = torch.maximum(
                ...
                (-G_stabilizer).exp()
                if _cfg["stabilize_correctly"]
                else torch.ones_like(G_stabilizer),
            )
        ...
        def backward(ctx, dh, dc_last=None, dn_last=None, dm_last=None):
            ...
            if use_chunkwise:
                if not _cfg["keep_c"]:
                    ...
                    kv = torch.einsum(
                        "bhctk,bhctd,bhct->bhckd", k, v, forward_gates.exp()
                    )
                    ksum = torch.einsum("bhctk,bhct->bhck", k, forward_gates.exp())
                ...
                for j in range(C, 0, -1):
                    dc[:, :, j - 1] = (
                        lflast[:, :, j - 1] + m[:, :, j - 1] - m[:, :, j]
                    ).exp()[..., None, None] * dc[:, :, j] + torch.einsum(
