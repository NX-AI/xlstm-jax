...
try:
    from xlstm_jax.kernels.mlstm_chunkwise.triton_stablef.triton_fwbw import _mlstm_chunkwise_fw
except ImportError:
...
def test_mlstm_chunkwise_fw_vs_jax_backends(
    ...
    h_out_triton, n_out, m_out, _, _ = _mlstm_chunkwise_fw(
        matQ=q, matK=k, matV=v, vecI=igate_preact, vecF=fgate_preact, chunk_size=CHUNK_SIZE, stabilize_correctly=True
    )
    ...
    np.testing.assert_allclose(
        ...
        atol=1e-2,
        rtol=1e-2,
        err_msg="Mismatch between JAX fwbw and JAX parallel.",
    )

    np.testing.assert_allclose(
        h_out_triton[0, 0], h_out_jax_fwbw[0, 0], atol=1e-2, rtol=1e-2, err_msg="Mismatch between Triton and JAX fwbw."
    )
    np.testing.assert_allclose(
