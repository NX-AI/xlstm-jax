...
try:
    from xlstm_jax.kernels.mlstm_chunkwise.triton_stablef.triton_fwbw import mlstm_chunkwise_triton_stablef
except ImportError:
    mlstm_chunkwise_triton_stablef = None
...
def test_mlstm_chunkwise_fw_full_module(default_qkvif: tuple[jax.Array, jax.Array, jax.Array, jax.Array, jax.Array]):
    ...
    h_out_triton = mlstm_chunkwise_triton_stablef(q, k, v, igate_preact, fgate_preact, chunk_size=CHUNK_SIZE)
...
@pytest.mark.parametrize("seed", [0, 42])
@pytest.mark.parametrize(["B", "NH", "S", "DH", "CHUNK_SIZE"], COMBINATIONS)
def test_mlstm_chunkwise_fwbw_vs_pytorch_fwbw_different_qkdim(
    B: int, NH: int, S: int, DH: int, CHUNK_SIZE: int, seed: int
):
    """Test the Triton kernels against the PyTorch fwbw backend."""
    config = mLSTMfwbwConfig_torch(
        chunk_size=CHUNK_SIZE,
    ...
    fwbw_torch_module = mLSTMfwbw_torch(config)
    _compare_to_pytorch_fn(B, NH, S, DH, CHUNK_SIZE, seed, fwbw_torch_module, DK=DH // 2)
...
def _compare_to_pytorch_fn(
    ...
    q = rng.normal(size=(B, NH, S, DK)).astype(np.float32)
    k = rng.normal(size=(B, NH, S, DK)).astype(np.float32)
    ...
    def loss_fn(q, k, v, igate_preact, fgate_preact):
        h_out_triton = mlstm_chunkwise_triton_stablef(
...
# zero of roughly 0.15, whereas - without an initial state - they should be at zero
# exactly, which is the case for the triton kernels
